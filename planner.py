...
async def detect_planning_mode(
    ...
    Args:
        ...
        client: Ollama async client for classification.
...
async def _classify_planning_mode_llm(
    ...
    """Use the local model to classify request and identify missing info."""
    try:
...
class TaskPlanner:
    ...
    async def _classify_request(self, text: str, client: OllamaClient) -> dict:
        """Use the local model to classify request type and extract known info."""
        try:
            response = await client.chat.completions.create(
                model="phi3:mini",
                max_tokens=200,
