...
def remember(content: str, mem_type: str = "fact", source: str = "", importance: int = 5) -> int:
    ...
    if _last is None:
        conn.rollback()
        conn.close()
        raise RuntimeError("Failed to insert memory, no row id returned")
    mem_id = int(_last)
    # Update FTS
    conn.execute(
...
def create_note(content: str, title: str = "", topic: str = "", tags: list[str] | None = None) -> int:
    ...
    if _last is None:
        conn.rollback()
        conn.close()
        raise RuntimeError("Failed to create note, no row id returned")
    note_id = int(_last)
    conn.execute(
...
async def extract_memories(user_text: str, jarvis_response: str, anthropic_client) -> list[str]:
    ...
    Uses the local model to decide if anything in the exchange is worth storing.
